...
(defmacro parse-then-map-result ((viterbi-node text start end tagger) &body body)
  (let ((result (gensym))
	(charseq (gensym))
	(code-stream (gensym)))
...
(defun parse (text &key (tagger *tagger*) 
  ...
  (parse-then-map-result (vn text start end tagger)
    (list (subseq text (+ start (vn:start vn)) (+ start (vn:end vn)))
...
(defun wakati (text &key (tagger *tagger*) (start 0) (end (length text)))
  (declare (optimize (speed 3) (debug 1) (safety 1))
	   ...
	   (charseq:index start end))
  (parse-then-map-result (vn text start end tagger)
    (subseq text (+ start (vn:start vn)) (+ start (vn:end vn)))))